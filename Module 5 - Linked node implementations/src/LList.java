public class LList<T extends Comparable<? super T>> implements ListInterface<T> {
	...
	public T getMax() 
	{
		...
		{
			T highest = firstNode.data;
			Node currentNode = firstNode.next;
			while (currentNode != null)
			{
				if (highest.compareTo(currentNode.data) < 0) //obj1 < obj2
				{
					highest = currentNode.data;
					currentNode = currentNode.next;
				}
				else
					currentNode = currentNode.next;
	...
	public void addAll(T[] items) 
	{
		if (isEmpty() && items.length > 0)
		{
			...
			firstNode = new Node(items[0]); //sets firstNode position
			Node currentNode = firstNode;
			numberOfEntries++;
		...
		else if (isEmpty() && items.length ==0) //if both list and array are empty
		{
			return;
		}
		else 
		{
			Node currentNode = firstNode;
			while(currentNode.next != null) //brings currentNode pointer to the last node. break once there.
			{
				currentNode = currentNode.next;
			}
			//currentNode will be at last node after exiting while loop. if loop does not execute then list is a singleton.
