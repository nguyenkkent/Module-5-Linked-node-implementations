...
public class HomeworkM5Driver {
	...
	public static void printAtInterval(Node<String> firstNode, int interval) {
		...
		if (firstNode == null) //if node is empty
			System.out.print("");
		
		else if(firstNode.next == null) //singleton, no need to validate interval
	...
	public static boolean isDoublyLinkedPalindromeList(DoubleNode<Integer> firstNode, DoubleNode<Integer> lastNode)  {
		if (firstNode == null || lastNode == null) //deals with empty list
			return true;
		else if (firstNode.equals(lastNode)) //deals with singleton
			return true;
		...
		{			
			...
			{
				if ((firstNode.data).equals(lastNode.data) == true)
				{
					firstNode = firstNode.next;
					lastNode = lastNode.previous;
				}
				else  //(firstNode.next.equals(lastNode.previous) == false)
				{
					return false;
				}
			}
	...
	public static void main(String[] args) {
		...
		System.out.println("If the elapsed time is much higher than that, or if your program stalls, you might revisit your code to see if you have a nested loop.");	
	...
	public static <T extends Comparable<? super T>> void testRemoveAll(T[] valuesToAdd, T valueToRemove, String testDescription) {
		...
		try {
			actualNumberRemoved = originalBag.removeAll(valueToRemove);
		} catch(NullPointerException ex) {
			ex.printStackTrace();
			System.out.println("\n**********TEST FAILED with an a NullPointerException.");
			System.out.println("       A NullPointerException was caught. This is an error because the code should not crash in that way.\n");
		...
		try {
			...
			System.out.println("          Error: There are nulls in the array that are being considered part of the bag.");
			System.out.println("                 Check that you are correctly updating the size of the bag.");	
			allTestsPassed = false;
			return;
		}
